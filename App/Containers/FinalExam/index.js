...
const FinalExam = (props) => {
    ...
    const getScore = () => {
      ...
      const mids     = props.navigation.state.params.lid;
      const excerpt  = props.navigation.state.params.excerpt;

      setMid(props.navigation.state.params.lid);
    ...
    //cek disini
    const getSoal = () => {
          let idm = props.navigation.state.params.lid;
          let exc = props.navigation.state.params.excerpt;
          let destiny = '';
          if(exc=='Final Test' || exc=='Grammer Test'){
              destiny='finex'
          }else if(exc=='Ujian Pelajaran' || exc=='Ujian Grammer'){
              destiny='subexam'
          }
          console.log('idm: '+idm+' | exc : '+ exc+ '| destiny : '+destiny);

          setState(state => ({...state, loading: true }))
          axios.get(svr.url+destiny+'/'+idm+'/'+exc+'/'+svr.api)
          .then(result=>{
    ...
    const getLastest = () => {
      ...
      console.log('ah--->'+svr.url+'lastest/get/'+mids+'/'+uids+'/'+excerpt+'/'+svr.api);
      setState(state => ({...state, loading: true }))
    ...
    const pushScore = () => {
      ...
      //let wrongAnswer = allQuestions.length-score;
      let excerpts    = props.navigation.state.params.excerpt;
      let body = {
       uid: state.id,
       mid: mid,
       skor: correct_answ,
...
//Multiselected-------------------------------------------------------------------------------
    ...
    // Memberikan nilai skoor pada validate selected multi  ---- ve
    ...
    const Draxzone= ({item, index}) =>{
      ...
      return(
        <View style={{flexDirection:'row', justifyContent:'space-around', width:'100%', marginBottom:20}}>
            <DraxView
                style={[styles.receiver,{minWidth:toDp(50)}]}
                key={index}
...
//DragDrop---------------------------------------------------END-----------------------------
    ...
    return (
       ...
       <Header
        title={props.navigation.state.params.title +' - ' +  props.navigation.state.params.excerpt}
